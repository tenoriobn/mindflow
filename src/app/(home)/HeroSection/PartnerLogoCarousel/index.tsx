...
const PartnerLogoCarousel = () => {
  return (
    ...
    >
      <div aria-hidden="true" className="partner-mask overflow-hidden opacity-0">
        <div className="partner-track 3xl:gap-[1.667vw] flex w-max items-center gap-4 md:gap-8">
          {[...partnerLogos, ...partnerLogos].map(({ name, Logo }, index) => (
            <div
              key={`${name}-${index}`}
              className="partner-tracks flex shrink-0 items-center justify-center"
            >
              <Logo className="3xl:h-[2.084vw] h-10 w-auto" aria-hidden="true" focusable="false" />
            </div>
          ))}
        </div>
      </div>
